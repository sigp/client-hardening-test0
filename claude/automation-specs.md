...
- **CRITICAL: NEVER WORK ON MAIN BRANCH!** Before you make ANY changes to the repo, always make sure you're in the appropriate branch. E.g. don't start implementing a new simulation on the `main` branch. FOR SIMULATIONS: Create branch `simulation/{simulation-name}` FIRST.
